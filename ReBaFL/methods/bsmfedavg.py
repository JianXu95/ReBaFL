...
class LocalUpdate_BSMFedAvg(object):
    ...
    def balanced_softmax1(self, logits, py):
        py1 = py + 1e-9
        py1 = py1.type_as(logits)
        py_smooth = py1.unsqueeze(0).expand(logits.shape[0], -1)
        logits = logits + py_smooth.log()
        return logits
    ...
    def restricted_softmax(self, logit, py):
        # py1 = 1.0*(py.gt(0.0).float()) + 0.3*(py.eq(0.0).float())
        py1 = 10.0*(py)
    ...
    def local_training(self, local_epoch, round=0):
        ...
        if local_epoch>0:
            for ep in range(local_epoch):
                ...
                for it in range(iter_num):
                    ...
                    # logits = self.balanced_softmax(output, self.Py)
                    # loss = F.cross_entropy(input=logits, target=labels)
                    loglikelihood = torch.log(self.balanced_softmax(output, self.Py))
        ...
        # multiple local iterations, but less than 1 epoch
            ...
            for it in range(iter_num):
                ...
                model.zero_grad()
                _, output = model(images)
                logits = self.balanced_softmax1(output, self.Py)
                loss = F.cross_entropy(input=logits, target=labels)
                loss.backward()
                optimizer.step()
                iter_loss.append(loss.item())
