...
class LocalUpdate_FedLC(object):
    def __init__(self, idx, args, train_set, test_set, g_test_set, model):
        ...
        self.ny = self.Py * len(self.train_data.dataset)
    ...
    def calibrated_softmax(self, logits, ny):
        tau = 1.0
        ny1 = ny + 1e-9
        ny1 = ny1.type_as(logits)
        ny_smooth = ny1.unsqueeze(0).expand(logits.shape[0], -1)
        logits = logits - tau*ny_smooth.pow(-0.25)
        return logits
        ...
        if local_epoch>0:
            for ep in range(local_epoch):
                ...
                for it in range(iter_num):
                    ...
                    logits = self.calibrated_softmax(output, self.ny)
