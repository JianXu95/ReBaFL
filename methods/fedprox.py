...
class LocalUpdate_FedProx(object):
    ...
    def local_training(self, local_epoch):
        ...
        grad_accum = []
        mu = self.mu
        global_param = list(copy.deepcopy(model).parameters()) 
        ...
        if local_epoch>0:
            for ep in range(local_epoch):
                ...
                for it in range(iter_num):
                    ...
                    # loglikelihood = torch.log(self.balanced_softmax(output))
                    # loss = self.nllloss(loglikelihood, labels)
                    loss = self.criterion(output, labels)
                    fed_prox_reg = 0.0
                    for param_index, param in enumerate(model.parameters()):
                        fed_prox_reg += ((mu / 2) * torch.norm((param - global_param[param_index]))**2)
                    loss += fed_prox_reg
                    loss.backward()
                    optimizer.step()
